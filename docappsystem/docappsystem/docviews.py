...
def Patient_Appointment_Details_Remark(request):
    if request.method == 'POST':
        ...
        recommendedtest = request.POST.get('recommendedtest')
        ...
        # Format the phone number to E.164 with +91 country code
        try:
            phone_number = patientaptdet.mobilenumber
            parsed_phone_number = phonenumbers.parse(f"+91{phone_number}", "IN")
            ...
            sms_message = f'Your appointment status has been updated to: {status}. Your appointment is scheduled on apppointment time. If any queries please feel free to contact us.\nRemark: {remark}. Thank you\n Cosmos Hospitals'
            message = client.messages.create(
                body=sms_message,
                from_=settings.TWILIO_PHONE_NUMBER,  # Your Twilio number
                to=formatted_phone_number
            )
            messages.success(request, "Status updated and SMS sent successfully.")
        except Exception as e:
            messages.error(request, f"Status updated but failed to send SMS: {str(e)}")
